                 ...
                 rejecter:(RCTPromiseRejectBlock)reject)
{
  ...
  #ifdef DEBUG
    jsCodeLocation = [[RCTBundleURLProvider sharedSettings] jsBundleURLForBundleRoot:name fallbackResource:name];
  #else
    jsCodeLocation = [[NSBundle mainBundle] URLForResource:name withExtension:@"jsbundle"];
  #endif
  NSLog(@"starting Thread %@", [threadURL absoluteString]);


   RCTBridge *threadBridge = [[RCTBridge alloc] initWithBundleURL:jsCodeLocation
                                            moduleProvider:nil
                                             launchOptions:nil];
