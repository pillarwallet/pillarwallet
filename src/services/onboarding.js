...
// utils
import { getEnsName, isCaseInsensitiveMatch, logBreadcrumb } from 'utils/common';
...
export const getExistingServicesAccounts = async (privateKey: string): Promise<Account[]> => {
  ...
  if (etherspotAccounts) {
    etherspotMappedAccounts = await Promise.all(
      etherspotAccounts.map(async ({ address }) => {
   ...
   */
  return [...etherspotMappedAccounts];
};
