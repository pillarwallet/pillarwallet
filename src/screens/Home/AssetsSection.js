...
function AssetsSection({ showSideChains }: Props) {
  ...
  const renderBalanceItem = (title: string, balance: ?BalanceInfo) => {
  ...
  const renderChainInfo = (chainInfo: ?ChainInfo) => {
    ...
    return (
      <>
        {renderBalanceItem(t('wallet'), chainInfo.wallet)}
        {renderBalanceItem(t('deposits'), chainInfo.deposits)}
        {renderBalanceItem(t('investments'), chainInfo.investments)}
        {renderBalanceItem(t('pools'), chainInfo.pools)}
        {chainInfo.collectibles != null && <HomeListItem title={t('collectibles')} value={formattedCollectibles} />}
        {chainInfo.contacts != null && <HomeListItem title={t('contacts')} value={formattedContacts} />}
      </>
    );
  };
