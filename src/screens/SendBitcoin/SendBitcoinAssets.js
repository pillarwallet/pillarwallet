...
import { SEND_BITCOIN_AMOUNT, SEND_COLLECTIBLE_CONFIRM, SMART_WALLET_INTRO } from 'constants/navigationConstants';
...
class SendBitcoinAssetsScreen extends React.Component<Props, State> {
  state = {
    activeTab: TOKENS,
  };
  ...
  proceedSendingAsset(nextScreenAssetData: NextScreenAssetData) {
    ...
    this.props.navigation.navigate(SEND_BITCOIN_AMOUNT, {
      assetData: {
        token,
        contractAddress,
        decimals,
...
export default connect(combinedMapStateToProps, mapDispatchToProps)(SendBitcoinAssetsScreen);
