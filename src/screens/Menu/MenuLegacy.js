...
const Menu = ({
  ...
  const renderMenuItem = ({ item }) => {
    const { title, action, labelBadge, emoji, icon, iconColor, hidden } = item;
  ...
  const deleteWallet = () => {
    if (isBackedUp) {
      ...
      Alert.alert(t('alert.attemptToLogOutWithoutBackup.title'), t('alert.attemptToLogOutWithoutBackup.message'), [
...
const mapStateToProps = ({ user: { data: user }, wallet: { backupStatus } }: RootReducerState): $Shape<Props> => ({
  user,
  backupStatus,
});
