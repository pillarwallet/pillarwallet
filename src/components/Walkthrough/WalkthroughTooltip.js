...
const TooltipArrowHolder = styled.View`
  ...
  ${({ bottom }) => bottom || bottom === 0 ? 'bottom: 0.2px;' : ''};
...
export const WalkthroughTooltip = (props: Props) => {
  ...
  if (isAttached) {
    if (positionLeft) {
      horizontalPos.left = spacing.large;
      horizontalPos.right = null;
    } else {
      horizontalPos.left = null;
      horizontalPos.right = spacing.large;
    }
  ...
  if (isAttached) {
    ...
    verticalPos.top = stepYPos + stepItemHeight;
    verticalPos.bottom = null;
  } else {
    verticalPos.top = null;
  ...
  return (
    <TooltipWrapper {...horizontalPos} {...verticalPos} tooltipWidth={200}>
