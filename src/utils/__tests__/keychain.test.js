...
describe('keychain utils test', () => {
  it('Should return null for invalid data provided', () => {
    expect(getPrivateKey(1)).toBeNull();
    expect(getPrivateKey({})).toBeNull();
    expect(getPrivateKey(null)).toBeNull();
    expect(getPrivateKey({ testKey: 'testValue' })).toBeNull();
  });

  it('Should return privateKey for valid data provided', () => {
    expect(getPrivateKey({ privateKey: 'testKey' })).toEqual('testKey');
  });
});
