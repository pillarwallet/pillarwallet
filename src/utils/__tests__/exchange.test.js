...
describe('Exchange Utility function tests', () => {
  ...
  it('Should choose the best offer and calculate amount', () => {
    const offers = [{ askRate: 1.5 }, { askRate: 2.0 }];
    expect(getBestAmountToBuy([], '10')).toBeNull();
    expect(getBestAmountToBuy(offers, '10')).toBe('20');
    expect(getBestAmountToBuy(offers, '')).toBeNull();
  });
  it('Should validate exchange input field correctly', () => {
    expect(validateInput('42.42', {}, {}, '')).toBeTruthy();
    expect(validateInput('0', {}, {}, '')).toBeFalsy();
    expect(validateInput('10.', {}, {}, '')).toBeFalsy();
    expect(validateInput('test', {}, {}, '')).toBeFalsy();
    expect(validateInput('10', null, {}, '')).toBeFalsy();
    expect(validateInput('10', {}, {}, 'error')).toBeFalsy();
  });
  it('Should trigger search under certain conditions', () => {
    const assetEth = { value: 'ETH', assetBalance: '42.42' };
    const assetPlr = { value: 'PLR', assetBalance: '0.001' };
  ...
  it('Should get asset balance from fiat amount', () => {
    ...
    expect(getAssetBalanceFromFiat('USD', 0, 'error', 'PLR')).toBe(0);
