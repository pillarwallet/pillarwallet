// @flow
import erc20Tokens from 'testUtils/erc20TokensMock';
import {
  includes,
  findList,
} from 'utils/erc20TokensSearch';
    ...
    describe('when search value is set', () => {
      ...
      describe('and the token(s) can be found by name or symbol', () => {
        it('should return the array with token(s)', () => {
          expect(findList(erc20Tokens, 'PLR').length).toBe(1);
          expect(findList(erc20Tokens, 'Pillar').length).toBe(1);
        });
      });
      ...
      describe('and more then 20 token(s) can be found by address, name or symbol', () => {
        it('should return the array with 20 elements', () => {
          const tokens = [];
          for (let i = 0; i < 30; i++) {
            tokens.push(erc20Tokens[0]);
          }

          expect(findList(tokens, 'p').length).toBe(20);
        });

        describe('and the size is set', () => {
          it('should return the array with N elements', () => {
            const size = 3;
            expect(findList(erc20Tokens, 't', size).length).toBe(size);
          });
        });
      });
    });
