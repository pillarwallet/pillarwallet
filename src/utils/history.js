...
import { BNB } from 'constants/assetsConstants';
...
export const findTransactionAcrossAccounts = (
  ...
  return Object.keys(history)
    ...
    .find(
      (transaction) =>
        isCaseInsensitiveMatch(hash ?? '', transaction?.hash) ||
...
export const getHistoryEventsFromTransactions = (
  ...
  return orderBy(transactions, ['createdAt'], ['asc']).reduce(
    (
      ...
      let eventType = addressesEqual(toAddress, activeAccountAddress)
      ...
      const isBNB = historyEvent?.value?.symbol === BNB;
      if (isBNB) return [...historyEvents, historyEvent];

      if (!successfulOrPendingDeploymentEventAdded && addressesEqual(fromAddress, activeAccountAddress)) {
        const deploymentFee =
          gasPrice && addressesEqual(fee?.address, nativeAssetPerChain.ethereum.address)
        ...
        successfulOrPendingDeploymentEventAdded = [TRANSACTION_STATUS.CONFIRMED, TRANSACTION_STATUS.PENDING].includes(
          historyEvent.status,
        );
...
export const getHistoryEventsFromCollectiblesTransactions = (
  ...
  transactions.map(
    ({ _id, hash, batchHash, createdAt, from: fromAddress, to: toAddress, gasUsed, gasPrice, status, assetData }) => {
...
export const getCrossChainAccountHistory = (accountHistory: ChainRecord<Transaction[]>): Transaction[] =>
  Object.keys(accountHistory ?? {}).flatMap((chain) => accountHistory?.[chain] ?? []);
